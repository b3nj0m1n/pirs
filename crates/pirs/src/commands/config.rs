...
pub fn run(cwd: &Path) -> Result<()> {
    ...
    println!("resolved_pir_dir: {}", d.config.pir_path(&d.root).display());
    println!("templates.default: {:?}", d.config.templates.default);
    println!("templates.custom: {:?}", d.config.templates.custom);
    println!(
        "privacy.redaction_patterns: {:?}",
        d.config.privacy.redaction_patterns
    );
    println!(
        "privacy.sensitive_fields: {:?}",
        d.config.privacy.sensitive_fields
    );
